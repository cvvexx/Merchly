...
Когда контейнеры станут `healthy` — http://localhost:8086 (порт шлюза).
...
```mermaid
flowchart LR
    Browser["Браузер<br/>(Thymeleaf + Bootstrap 5)"] -->|"cookie сессии"| GW

    subgraph Compose["docker-compose.prod.yml"]
        GW["gateway-service :8086<br/>единственный вход, rate limiting"]
...
Ограничение частоты запросов живёт целиком на шлюзе — это единственная точка входа
снаружи, и там же лежит вся картина трафика. Реализация на Bucket4j: token bucket
с возвратом токенов **целым окном** (`refillIntervally`), а не по капле, иначе
терпеливый атакующий держит постоянный темп подбора.

Ключ выбирается по типу правила:

| Ключ         | Когда используется                              | Зачем именно так                                                  |
|--------------|--------------------------------------------------|--------------------------------------------------------------------|
| `IP`         | анонимные пути (каталог, отзывы, регистрация)   | у запроса нет пользователя, считать больше не по чему             |
| `USER`       | всё, что требует аутентификации                 | `sub` из **проверенного** JWT, не зависит от адреса               |
| `FORM_LOGIN` | `POST /do-login`, вместе с `IP`                 | защищает конкретный аккаунт от распределённого подбора            |

Два неочевидных момента, без которых схема не работает:

* **Фильтр стоит после аутентификации Spring Security**, а не перед ней. Если брать
  `sub` из `Authorization` самостоятельно, до проверки подписи, злоумышленник
  подставит произвольный `sub` в неподписанный токен и получит свежий бакет на каждый
  запрос. После `AuthorizationFilter` в `SecurityContextHolder` лежит уже проверенный
  principal.
* **Запросы из внутренней сети не считаются по IP.** `frontend-service` ходит в API
  обратно через шлюз, поэтому на втором заходе адрес источника — это адрес контейнера
  BFF. Лимит по IP на `/api/**` посадил бы весь сайт в один бакет. Такие запросы
  считаются по пользователю, а анонимные уже посчитаны на браузерном крае.

Основные лимиты (полная таблица — в `RateLimitRules`):

| Что                                        | Ключ       | Лимит          |
|--------------------------------------------|------------|----------------|
| `POST /do-login`                           | IP + логин | 10/5м и 5/15м  |
| `POST /do-register`, `/api/users/register`  | IP         | 3/час          |
...
на остальном — `429` с заголовком `Retry-After`.

Настройки:

| Свойство                                 | По умолчанию                  | Смысл                                                                 |
|------------------------------------------|-------------------------------|-------------------------------------------------------------------------|
| `merchly.ratelimit.trust-forwarded-for`  | `false`                       | учитывать `X-Forwarded-For`; включать **только** если перед шлюзом реально стоит прокси, иначе лимит обходится подделкой заголовка |
| `merchly.ratelimit.internal-networks`    | приватные диапазоны + loopback | какие адреса считаются внутренними                                    |
...
* вход реализован через direct access grant, а не через redirect-флоу
  `authorization_code` — форма логина живёт на шлюзе;
* бакеты rate limiting лежат в памяти процесса шлюза. Сейчас это работает честно —
  шлюз запускается одним инстансом, — но вторая реплика молча удвоит лимиты.
